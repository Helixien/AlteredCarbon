...
namespace AlteredCarbon
{
    public class WorkGiver_HaulStacks : WorkGiver_Scanner
    {
        ...
        {
            return Building_StackStorage.building_StackStorages.Any(x => x.HasFreeSpace && x.Accepts(t));
        }
        public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
        {
            var containers = Building_StackStorage.building_StackStorages.Where(x => x.HasFreeSpace 
                && x.Accepts(t) && pawn.CanReserveAndReach(x, PathEndMode.Touch, Danger.Deadly));
            if (containers.Any())
            {
